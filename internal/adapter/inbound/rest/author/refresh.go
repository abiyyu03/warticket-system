...
func (h *Handler) Refresh(fctx *fiber.Ctx) error {
	...
	resp, err := h.Service.Author.Refresh(ctx, request.ToUcEntity())
	if err != nil {
	...
	return fctx.Status(fiber.StatusOK).JSON(
		baseEntity.BaseResponse{}.ToResponse("Token Refreshed Successfully", fiber.StatusOK, resp, nil),
	)
}
